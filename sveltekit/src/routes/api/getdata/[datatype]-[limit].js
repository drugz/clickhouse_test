...
export async function get({ params }) {
	...
	try {
		const handlers = {
			...
			cart_products: async () => {
				return await clickhouse.queryPromise(`SELECT cart_products.id, cart_products.amount, cart_products.product_id, products.price, products.no_discount_flag FROM cart_products INNER JOIN (SELECT * FROM products limit 1000) AS products ON (cart_products.product_id = products.id)`);
			},
